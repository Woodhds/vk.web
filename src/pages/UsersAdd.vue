<template>
  <q-form class="q-gutter-md q-pb-md"
          style="max-width: 450px" @submit.prevent="makeSearch">
    <q-input dense outlined v-model="search" label="Поиск"/>
    <q-btn color="primary" type="submit" label="Отправить">
    </q-btn>
  </q-form>
  <div class="column">
    <div v-for="user in users" :key="user.id" class="row q-gutter-sm items-center">
      <q-avatar>
        <q-img :src="user.avatar"/>
      </q-avatar>
      <q-btn :href="`https://vk.com/id${user.id}`" flat dense no-caps target="_blank" :label="user.name"></q-btn>
